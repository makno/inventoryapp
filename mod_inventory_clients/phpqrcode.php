 ...
 */
    ...
    class QRvect {
        ...
        //----------------------------------------------------------------------
        private static function vectSVGdeprecated($frame, $pixelPerPoint = 4, $outerFrame = 4, $back_color = 0xFFFFFF, $fore_color = 0x000000) 
        {
            $h = count($frame);
            $w = strlen($frame[0]);
	    ...
	    //----------------------------------------------------------------------
		// method by MaKno 2013 - Usable for Firefox, Chrome as well!
        private static function vectSVG($frame, $pixelPerPoint = 4, $outerFrame = 4, $back_color = 0xFFFFFF, $fore_color = 0x000000) 
        {
			...
			if(!empty($back_color)) {
				...
				$backgroundcolor = dechex(0x000000);
            }
			
            $output = 
			'<svg  xmlns="http://www.w3.org/2000/svg" version="1.1" width="'.$imgW * $pixelPerPoint.'" height="'.$imgH * $pixelPerPoint.'">'."\n".
			'<rect x="0" y="0" width="'.$imgW * $pixelPerPoint.'" height="'.$imgH * $pixelPerPoint.'" style="fill:#'.$backgroundcolor.';stroke:none;stroke-width:0;"/>'."\n";
                
            // Convert the matrix into rectangles
            for($i=0; $i<$h; $i++) {
                for($j=0; $j<$w; $j++) {
                    if( $frame[$i][$j] == '1') {
                        $y = ($i + $outerFrame) * $pixelPerPoint;
                        $x = ($j + $outerFrame) * $pixelPerPoint;
                        $output .= '<rect x="'.$x.'" y="'.$y.'" width="'.$pixelPerPoint.'" height="'.$pixelPerPoint.'" style="fill:#'.$fore_color.'" />'."\n";
                    }
                }
            }
            $output .= 
